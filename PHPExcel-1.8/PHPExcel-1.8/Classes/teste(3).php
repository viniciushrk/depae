    ...
    for ($linha = 1; $linha<=$total_linhas; $linha += (34 + $numeroDeDisciplinas)) {
        ...
        if(isset($materias)){
            ...
            for ($c = 0; $c < $numeroDeDisciplinas; $c++) {
                $query = $disciplina->selecionaDisciplinasPorMateriaECargaHoraria($materias->offsetGet($c), $cargas_horarias->offsetGet($c), $curso->getIdCurso());

                if (is_array($query)) {
                    $discAluno->setDisciplinaIdDisciplina($query[0]['idDisciplina']); //arrumar
                }else{
                    $disciplina->setMateria($materias->offsetGet($c));
